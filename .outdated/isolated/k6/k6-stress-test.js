...
// Load test parameters - aggressive stress test
// Ramp up: 20 users every 3s (rapid stress)
// Hold: 90 seconds at max
// Ramp down: 30 seconds
// Total: ~3 minutes max
const RAMPUP_VUS = USERS_COUNT * 2;      // Number of concurrent users at max (2x USERS_COUNT)
const RAMP_INCREMENT = 20;                // Add 20 users per step
const STEP_DURATION = '10s';
const STEADY_DURATION = '90s';            // Hold for 90 seconds at max
const RAMPDOWN_DURATION = '30s';          // Quick ramp down

// Generate stages dynamically: rapid ramp-up to max, hold, quick ramp-down
function generateStages() {
  const stages = [];
  
  // Ramp up: 20 users at a time, every 3 seconds (aggressive)
  for (let vus = RAMP_INCREMENT; vus <= RAMPUP_VUS; vus += RAMP_INCREMENT) {
    stages.push({ duration: STEP_DURATION, target: vus });
  }
  
  // Hold at max for 90 seconds
  stages.push({ duration: STEADY_DURATION, target: RAMPUP_VUS });
  
  // Quick ramp down to 0
...
// Load test configuration with stress test parameters
export const options = {
  stages: generateStages(),
  thresholds: {
    // Stress test: Break on performance degradation
    'http_req_duration{phase:test}': [
      { threshold: 'p(95)<300', abortOnFail: true },   // Abort if P95 > 300ms
      { threshold: 'p(99)<600', abortOnFail: true },   // Abort if P99 > 600ms
    ],
    http_req_failed: ['rate<0.01'],                    // Allow up to 5% failure during stress
  },
  gracefulStop: '30s',
  gracefulRampDown: '30s',
  maxDuration: '3m',                                   // Hard stop at 3 minutes
};
...
export default function(data) {
  ...
  const userIdx = __VU-1 >= USERS_COUNT ? (__VU-1) % USERS_COUNT : __VU-1;
  ...
  // Operation mix: 10% create, 60% list/filter, 30% update
...
function createIssueOperation(user, bearerToken, project, sprintMap) {
  ...
  if (issueResponse.status !== 201 && issueResponse.status !== 403) {
...
function listIssuesOperation(user, bearerToken, project, sprintMap) {
  ...
  if (listResponse.status !== 200 && listResponse.status !== 403) {
...
function updateSpecificIssue(user, bearerToken, issue, project) {
  ...
  if (updateResponse.status !== 200 && updateResponse.status !== 403) {
