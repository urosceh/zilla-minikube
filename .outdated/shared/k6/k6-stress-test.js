...
const USERS_COUNT = __ENV.USERS_COUNT || 200;

// Stress test parameters - gradually increase users
const INITIAL_VUS = 5;
const VU_INCREMENT = 5;
const INCREMENT_INTERVAL = 3;
const MAX_VUS = USERS_COUNT;
...
// Generate stages for stress test
function generateStages() {
  const stages = [];
  for (let vus = INITIAL_VUS; vus <= MAX_VUS; vus += VU_INCREMENT) {
    stages.push({ duration: `${INCREMENT_INTERVAL}s`, target: vus });
  }
  // Hold at max for 1 minute
  stages.push({ duration: '1m', target: MAX_VUS });
  return stages;
}

// Stress test configuration with ramping VUs
...
export default function(data) {
  ...
  if (!userTokenCache[user.userId]) {
    ...
    if (loginResponse.status !== 200) {
      console.error(`[${TENANT}] User login failed: ${user.email}`);
      return;
    }
    ...
    try {
      ...
      console.error(`[${TENANT}] Failed to parse login response`);
      return;
    }
  }
  ...
  if (userProjects.status !== 200) {
    console.error(`[${TENANT}] Failed to get projects`);
    return;
  }
  ...
  if (!userProjectsList || userProjectsList.length === 0) {
    console.error(`[${TENANT}] User has no projects`);
    return;
  }

  // Stress test - high throughput with many operations
...
function createIssueOperation(user, bearerToken, project, sprintMap) {
  ...
  const issueResponse = http.post(`${BASE_URL}/api/issue`, JSON.stringify({
    ...
    details: `Created during stress test`,
  ...
  if (issueResponse.status !== 201) {
    console.error(`[${TENANT}] Failed to create issue`);
  }
}

function listIssuesOperation(user, bearerToken, project, sprintMap) {
  const offset = randomIntBetween(0, 50);
  ...
  if (listResponse.status !== 200) {
    console.error(`[${TENANT}] Failed to list issues`);
  }
  ...
  if (listResponse.status === 200) {
    ...
    if (issues && issues.length > 0) {
      ...
      if (issue && issue.issueId && randomIntBetween(0, 100) > 50) {
...
function updateIssueOperation(user, bearerToken, project) {
  ...
  if (listResponse.status === 200) {
    ...
    console.error(`[${TENANT}] Failed to list issues for update`);
  }
}
...
function updateSpecificIssue(user, bearerToken, issue, project) {
  ...
  if (updateResponse.status !== 200) {
    console.error(`[${TENANT}] Failed to update issue`);
  }
}

export function teardown(data) {
  console.log(`[${TENANT}] Stress test completed`);
}
